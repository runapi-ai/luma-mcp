<h1 align="center">Luma MCP Server</h1>

<p align="center">
  <strong>Create Luma AI tasks through RunAPI — 1 endpoint, 1 model — then poll status and check pricing. One MCP server, one API key.</strong>
</p>
...
`@runapi.ai/luma-mcp` is a focused MCP server for the **Luma** model line on RunAPI.
It lets an MCP-compatible assistant create Luma tasks, poll their status, and check current pricing — all through a single RunAPI API key.
...
| Tool | Auth | Purpose |
|---|---|---|
| `modify_video` | Yes | Create a Luma task (modify video) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `get_task` | Yes | Fetch the current status and latest payload for an existing task. |
| `check_pricing` | No | Look up the current price for a Luma model and endpoint. |

---

## Models

Luma covers 1 model across 1 endpoint. Each tool accepts the models listed for it:
...
The assistant calls `check_pricing` to confirm the cost, then `modify_video`, and returns the task id, status, and output URLs.
